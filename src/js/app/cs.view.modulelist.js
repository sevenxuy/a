  ...
  $.widget('cs.modulelist', {
    ...
    render: function() {
      ...
      $.ajax({
        ...
        self.element.append(self._createModulelistElem(data.data));
    ...
    _createModulelistElem: function(data) {
      var self = this,
        options = this.options,
        h = [];
      ...
      _.each(data, function(item, index) {
        h.push(self._createModuleElem(item));
      });
      h.push('</tbody></div>');
      return h.join('');
    },
    _createModuleElem: function(item) {
      var h = [],
        options = this.options;
    ...
    _dataAddElem: function(event) {
      ...
      h.push('<a class="btn btn-link data-del">删除</a>');
      h.push('</td></tr>');
      $('#modulelist').find('tbody').prepend(h.join(''));
      ...
      $.ajax({
        ...
        if (!response.errno) {
          ...
          if (res == true) {
            ...
            $tr.remove();
            self.element.find('tbody').prepend(self._createModuleElem(res));
          }
        } else {
          notify({
            text: response.error
